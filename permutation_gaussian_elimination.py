...
class SparseGraphFormat:
    ...
    def _bfs(self, v: Vertex, q: Queue, ordering: List[Vertex]):
        ...
        while not q.empty():
            u = q.get()
            ordering.append(u)
            neighbours = sorted([self.vertices[i] for i in u.row])
            for n in neighbours:
                if not n.visited:
                    n.visited = True
                    q.put(n)
    ...
    def cuthil_mckee_ordering(self):
        ...
        for vertex in self.vertices:
            reordered_vertices[mapping[vertex.id]].as_row = {mapping[key]: value for key, value in
                                                             vertex.as_row.items()}
            reordered_vertices[mapping[vertex.id]].as_column = {mapping[key]: value for key, value in
                                                                vertex.as_column.items()}
